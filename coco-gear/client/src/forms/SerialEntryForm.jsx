...
function SerialEntryForm({kit,type,allC,existingSerials,mode,onDone,onCancel,settings}){
  ...
  const filled=serComps.length===0||serComps.every(c=>serials[c._key]&&serials[c._key].trim());
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        {serComps.map(c=>{const ex=existingSerials&&existingSerials[c._key];const lbl=c._qty>1?c.label+" ("+(c._idx+1)+" of "+c._qty+")":c.label;return(
          <div key={c._key} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:8,background:"rgba(251,191,36,.02)",border:"1px solid rgba(251,191,36,.1)"}}>
            <div style={{flex:1}}>
              ...
              {ex&&<div style={{fontSize:9,color:T.mu,fontFamily:T.m,marginBottom:3}}>Expected: {ex}</div>}
            ...
            <span style={{color:serials[c._key]?.trim()?(ex&&serials[c._key].trim()===ex?T.gn:ex?T.rd:T.gn):T.rd,fontSize:12,fontWeight:700}}>{serials[c._key]?.trim()?(ex&&serials[c._key].trim()===ex?"✓":ex?"✗":"✓"):"--"}</span></div>)})}</div></div>}
    ...
    <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><Bt onClick={onCancel}>Cancel</Bt>
      <Bt v={mode==="checkout"?"primary":mode==="return"?"warn":"success"} onClick={()=>onDone({serials,notes})} disabled={serComps.length>0&&!filled}>
        {serComps.length>0&&!filled?"Enter all S/N":"Confirm "+ml}</Bt></div></div>);}
