...
function TypeAdmin({types,setTypes,comps,kits,depts,onRefreshTypes}){
  const[md,setMd]=useState(null);const[fm,setFm]=useState({name:"",desc:"",compIds:[],compQtys:{},criticalCompIds:[],fields:[],deptIds:[]});const[fd,setFd]=useState({key:"",label:"",type:"text"});
  const[deleteConfirm,setDeleteConfirm]=useState(null);
  const grouped=useMemo(()=>{const g={};comps.forEach(c=>{(g[c.cat]=g[c.cat]||[]).push(c)});return g},[comps]);
  const togC=cid=>{setFm(p=>{if(p.compIds.includes(cid)){const nq={...p.compQtys};delete nq[cid];return{...p,compIds:p.compIds.filter(x=>x!==cid),compQtys:nq,criticalCompIds:(p.criticalCompIds||[]).filter(x=>x!==cid)}}return{...p,compIds:[...p.compIds,cid]}})};
  const togCritical=cid=>{setFm(p=>{const cur=p.criticalCompIds||[];return{...p,criticalCompIds:cur.includes(cid)?cur.filter(x=>x!==cid):[...cur,cid]}})};
  ...
  const save=async()=>{if(!fm.name.trim())return;
    const components=fm.compIds.map(id=>({componentId:id,quantity:fm.compQtys[id]||1,critical:(fm.criticalCompIds||[]).includes(id)}));
  ...
  return(<div>
    <SH title="Kit Types" sub={types.length+" templates"} action={<Bt v="primary" onClick={()=>{setFm({name:"",desc:"",compIds:[],compQtys:{},criticalCompIds:[],fields:[],deptIds:[]});setMd("add")}}>+ Add</Bt>}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(300px,100%),1fr))",gap:12}}>
      {types.map(t=>{const sc=t.compIds.filter(id=>comps.find(c=>c.id===id&&c.ser)).length;const inUse=kits?.filter(k=>k.typeId===t.id).length||0;const tc=totalExpanded(t.compIds,t.compQtys||{});const critCt=(t.criticalCompIds||[]).length;return(
        <div key={t.id} style={{padding:18,borderRadius:10,background:T.card,border:"1px solid "+T.bd}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:4}}><Bt v="ghost" sm onClick={()=>{setFm({name:t.name,desc:t.desc,compIds:[...t.compIds],compQtys:{...(t.compQtys||{})},criticalCompIds:[...(t.criticalCompIds||[])],fields:t.fields.map(f=>({...f})),deptIds:[...(t.deptIds||[])]});setMd(t.id)}}>Edit</Bt>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            ...
            {critCt>0&&<Bg color={T.rd} bg="rgba(239,68,68,.08)">{critCt} critical</Bg>}
        ...
        <div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:1.2,color:T.mu,fontFamily:T.m,marginBottom:8}}>Components ({totalExpanded(fm.compIds,fm.compQtys)} items, {fm.compIds.length} types)</div>
          {Object.entries(grouped).map(([cat,items])=><div key={cat} style={{marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
              {items.map(c=>{const s=fm.compIds.includes(c.id);const q=fm.compQtys[c.id]||1;const isCrit=(fm.criticalCompIds||[]).includes(c.id);return(
                <div key={c.id} style={{display:"flex",alignItems:"center",gap:2}}>
                  <button onClick={()=>togC(c.id)} style={{all:"unset",cursor:"pointer",padding:"4px 10px",borderRadius:s?(q>1||true)?"5px 0 0 5px":5:5,fontSize:10,fontFamily:T.m,
                  ...
                  {s&&<div style={{display:"flex",alignItems:"center",background:"rgba(129,140,248,.08)",border:"1px solid rgba(129,140,248,.35)",borderLeft:"none",overflow:"hidden"}}>
                  ...
                  {s&&<button onClick={()=>togCritical(c.id)} title={isCrit?"Remove critical flag":"Mark as critical"} style={{all:"unset",cursor:"pointer",padding:"4px 8px",
                    borderRadius:"0 5px 5px 0",fontSize:9,fontFamily:T.m,fontWeight:600,
                    background:isCrit?"rgba(239,68,68,.15)":"rgba(255,255,255,.03)",border:"1px solid "+(isCrit?"rgba(239,68,68,.35)":T.bd),borderLeft:"none",
                    color:isCrit?T.rd:T.dm}}>{isCrit?"CRIT":"crit"}</button>}
                </div>)})}</div></div>)}</div>
